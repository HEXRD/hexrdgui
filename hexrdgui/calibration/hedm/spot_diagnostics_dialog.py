...
class SpotDiagnosticsDialog:
    def __init__(
        ...
        det_keys: list[str],
        det_dims: dict[str, tuple[float, float]],
        det_tvecs: dict[str, np.ndarray],
    ...
    def update_enable_states(self) -> None:
        ...
        enable_detector = (
            not self.show_all_detectors and self.ui.detector.count() > 1
        )
        self.ui.detector.setEnabled(enable_detector)
        self.ui.detector_label.setEnabled(enable_detector)

        show_all_detectors_visible = self.ui.detector.count() > 1
    ...
    def update_data(
        ...
        """Update predicted data and refresh plots.

        Called after calibration to reflect updated model predictions,
        recomputed angular coordinates, and current detector translations.
        """
        self.xyo_pred = xyo_pred
        self.pred_angs = pred_angs
        self.meas_angs = meas_angs
    ...
    def _get_data_for_quantity(
        ...
        scatter_x = xyo_p[:, 0] + tvec[0]
        scatter_y = xyo_p[:, 1] + tvec[1]
    ...
    def update_canvas(self) -> None:
        ...
        if self.match_detector_shape:
            ...
            x_min = min(
                self.det_tvecs[k][0] - self.det_dims[k][0] / 2
                for k in det_keys
            )
            x_max = max(
                self.det_tvecs[k][0] + self.det_dims[k][0] / 2
            ...
            y_min = min(
                self.det_tvecs[k][1] - self.det_dims[k][1] / 2
                for k in det_keys
            )
            y_max = max(
                self.det_tvecs[k][1] + self.det_dims[k][1] / 2
